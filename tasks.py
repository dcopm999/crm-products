...
def coverage(c):
    ...
    """
    c.run("coverage run --source crm-products runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/crm-products.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ products")
...
def lint(c):
    ...
    """
    c.run("flake8 crm-products tests")
...
def release(c, bumpsize=''):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=crm_products.__version__))
    c.run("git push --tags")
